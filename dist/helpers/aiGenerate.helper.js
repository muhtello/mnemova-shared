...
exports.trimSourceText = trimSourceText;
exports.buildKeywordSegments = buildKeywordSegments;
...
function buildKeywordSegments(text, keywords) {
    const sorted = [...keywords].sort((a, b) => a.start - b.start);
    const segments = [];
    let cursor = 0;
    for (const kw of sorted) {
        if (kw.start < cursor || kw.start >= kw.end)
            continue; // skip overlaps/invalid
        if (kw.start > cursor)
            segments.push({ text: text.slice(cursor, kw.start), keywordId: null });
        segments.push({ text: text.slice(kw.start, kw.end), keywordId: kw.id });
        cursor = kw.end;
    }
    if (cursor < text.length)
        segments.push({ text: text.slice(cursor), keywordId: null });
    return segments;
}
// One-line answer/content preview per exercise type, for a generation review list.
function previewFor(exercise) {
    var _a;
    switch (exercise.type) {
        case "flashcard":
            return exercise.answer;
        case "fill-in-the-blank":
            return ((_a = exercise.blanks) !== null && _a !== void 0 ? _a : [exercise.blank]).filter(Boolean).join(", ");
        case "word-pick":
